...
def worker_process(rank, world_size, total_samples, config_dict, results_queue):
    ...
    try:
        ...
        print(
            f"Rank {rank}: Processing samples {start_id} to {end_id - 1} ({samples_to_process} samples)"
        )
        ...
        results_queue.put({"rank": rank, "status": "error", "error": str(e)})
    ...
    for result in results:
        if result["status"] == "completed":
            ...
            for sample in result["first_few_samples"]:
                print(
                    f"    Sample {sample['sample_id']}: rot={sample['rotation']:.2f}, "
...
def demo_parameter_uniqueness_verification():
    ...
    for rank in range(world_size):
        ...
        print(
            f"Rank {rank}: rotation={params['rotation']:.4f}, "
    ...
    for rank in range(world_size):
        ...
        for sample_id in range(start_id, end_id):
            params = gen_distributed_augmentation_params(
                sample_id, rank, config, world_size=world_size
            )
