...
from src.distributed import RankAwareSeedGenerator, gen_distributed_augmentation_params
...
def demo_rank_aware_generation():
    ...
    for rank in range(world_size):
        ...
        for sample_id in range(num_samples):
            params = gen_distributed_augmentation_params(
                sample_id=sample_id, rank=rank, config=config, world_size=world_size
            )
    ...
    for rank in range(world_size):
        ...
        print(
            f"Rank {rank}: rotation={params['rotation']:.2f}, "
            f"brightness={params['brightness']:.3f}, "
