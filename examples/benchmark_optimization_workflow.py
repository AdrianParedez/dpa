...
from src.benchmark import BenchmarkRunner, BenchmarkConfig, PerformanceProfiler, measure_time
from src.dpa import get_preset, AugmentationConfig, gen_augmentation_params, stream_augmentation_chain
...
class OptimizationWorkflow:
    ...
    def step2_identify_bottlenecks(self, num_samples=100):
        ...
        slowest_operation = max(bottleneck_results.items(), 
    ...
    def step4_batch_optimization(self, num_samples=200, optimal_config=None):
        ...
        batch_configs = {
            "small_sequential": BatchConfig(
                strategy=BatchStrategy.SEQUENTIAL,
                batch_size=5
            ),
            "medium_sequential": BatchConfig(
                strategy=BatchStrategy.SEQUENTIAL,
                batch_size=15
            ),
            "large_sequential": BatchConfig(
                strategy=BatchStrategy.SEQUENTIAL,
                batch_size=30
            ),
    ...
    def step5_fine_tuning(self, num_samples=100, optimal_config=None, optimal_batch_config=None):
        ...
        best_fine_tuned = max(fine_tune_results.items(), 
                             key=lambda x: x[1].throughput_samples_per_second)
    ...
    def generate_optimization_report(self):
        ...
        print(f"Baseline throughput: {baseline['metrics'].throughput_samples_per_second:.1f} samples/sec")
        ...
        for i, step in enumerate(self.optimization_history, 1):
            if i == 1:
                ...
                prev_throughput = self.optimization_history[i-2]['metrics'].throughput_samples_per_second
            ...
            print(f"{i}. {step['description']}: {step['metrics'].throughput_samples_per_second:.1f} samples/sec ({improvement})")
...
def demo_complete_optimization_workflow():
    ...
    optimal_batch_name, optimal_batch_config = workflow.step4_batch_optimization(num_samples * 2, optimal_config)
