...
from src.benchmark import PerformanceProfiler, measure_time, measure_memory
from src.dpa import get_preset, gen_augmentation_params, stream_augmentation_chain
...
def demo_context_manager_profiling():
    ...
    print(f"Using context managers to profile different operations:")
    ...
    print(f"   Batch throughput: {len(processed_batches) / timer['elapsed_seconds']:.1f} batches/sec")
...
def demo_comparative_profiling():
    ...
    for approach_name, result in results.items():
        if approach_name != "sequential":
            ...
            memory_ratio = result.memory_delta_mb / baseline.memory_delta_mb if baseline.memory_delta_mb != 0 else 1
...
def demo_detailed_operation_profiling():
    ...
    for operation, result in sorted_operations:
        percentage = (result.total_time_seconds / sum(r.total_time_seconds for r in profile_summary.values())) * 100
...
def demo_performance_regression_detection():
    ...
    if abs(performance_change) > regression_threshold:
        if performance_change > 0:
            ...
            print(f"   📈 Performance improvement detected! ({abs(performance_change):.1f}% faster)")
    else:
