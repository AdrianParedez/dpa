...
from src.benchmark import BenchmarkConfig, BenchmarkRunner, PerformanceMetrics
...
def demo_configuration_comparison():
    ...
    benchmark_config = BenchmarkConfig(
        iterations=50, warmup_iterations=5, measure_memory=True, measure_cpu=True
    )
    ...
    print(
        f"Fastest configuration: {best_throughput[0]} ({best_throughput[1].throughput_samples_per_second:.1f} samples/sec)"
    )
    print(
        f"Slowest configuration: {worst_throughput[0]} ({worst_throughput[1].throughput_samples_per_second:.1f} samples/sec)"
    )
    ...
    performance_ratio = (
        best_throughput[1].throughput_samples_per_second
        / worst_throughput[1].throughput_samples_per_second
    )
    ...
    if lowest_memory[1].memory_usage_mb > 0:
        ...
        print("Memory usage range: All configurations use minimal memory")
...
def demo_batch_strategy_comparison():
    ...
    batch_configs = {
        "sequential_small": BatchConfig(strategy=BatchStrategy.SEQUENTIAL, batch_size=5),
        "sequential_large": BatchConfig(strategy=BatchStrategy.SEQUENTIAL, batch_size=20),
        "round_robin": BatchConfig(strategy=BatchStrategy.ROUND_ROBIN, batch_size=10),
        "memory_optimized": BatchConfig(
            strategy=BatchStrategy.MEMORY_OPTIMIZED, max_memory_mb=100, min_batch_size=3
        ),
    ...
    benchmark_config = BenchmarkConfig(iterations=30, warmup_iterations=3, measure_memory=True)
    ...
    sorted_results = sorted(
        results.items(), key=lambda x: x[1].throughput_samples_per_second, reverse=True
    )
...
def demo_distributed_scaling_analysis():
    ...
    benchmark_config = BenchmarkConfig(iterations=20, warmup_iterations=2, measure_memory=True)
    ...
    for world_size in world_sizes:
        ...
        print(
            f"  Theoretical total throughput: {metrics.throughput_samples_per_second * world_size:.1f} samples/sec"
        )
        print()
    ...
    baseline_total_time = baseline.total_time_seconds * (
        total_samples / scaling_results[1]["samples_per_rank"]
    )
    ...
    for world_size in world_sizes[1:]:  # Skip world_size=1
        ...
        print(
            f"  World size {world_size:2d}: {actual_speedup:.1f}x speedup ({efficiency:.1f}% efficiency)"
        )
    ...
    if efficient_sizes:
        ...
        print(
            f"  Recommended world size: {best_efficient[0]} ({best_efficient[1]:.1f}% efficiency)"
        )
    else:
...
def demo_performance_trend_analysis():
    ...
    benchmark_config = BenchmarkConfig(iterations=25, warmup_iterations=3, measure_memory=True)
...
def demo_comprehensive_benchmark_report():
    ...
    for config_name, config_results in comprehensive_results.items():
        ...
        for sample_count, results in config_results.items():
            print(
                f"{sample_count:<8} {results['throughput']:<12.1f} "
