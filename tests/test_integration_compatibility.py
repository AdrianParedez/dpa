...
class TestBackwardCompatibility:
    ...
    def test_file_format_compatibility_with_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            save_augmentation_chain(params_list, str(batch_filepath), config=config, include_stats=True)
            ...
            save_augmentation_stream(generator, str(streaming_filepath), config=config, include_stats=True)
    ...
    def test_metadata_compatibility(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert batch_data["metadata"]["num_samples"] == streaming_data["metadata"]["num_samples"]
            ...
            assert "streaming" not in batch_data["metadata"] or batch_data["metadata"]["streaming"] is None
...
class TestIntegrationScenarios:
    ...
    def test_mixed_api_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            save_augmentation_chain(loaded_streaming, str(filepath2), config=config, include_stats=True)
    ...
    def test_preset_compatibility_comprehensive(self):
        ...
        for preset_name in ["mild", "moderate", "aggressive"]:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                save_augmentation_chain(batch_result, str(batch_file), config=config, include_stats=True)
                ...
                stream_loaded_from_batch = list(load_augmentation_stream(str(batch_file), chunk_size=1))
