...
class TestDistributedTrainingIntegration:
    ...
    def test_reproducibility_across_distributed_runs(self):
        ...
        for rank in range(world_size):
            ...
            for sample_id in range(start_id, end_id):
                ...
                assert params1 == params2, f"Non-reproducible parameters for rank {rank}, sample {sample_id}"
    ...
    def test_distributed_parameter_uniqueness_across_ranks(self):
        ...
        for i in range(world_size):
            for j in range(i + 1, world_size):
                ...
                different = (
                    params_i["rotation"] != params_j["rotation"] or
                    params_i["brightness"] != params_j["brightness"] or
                    params_i["noise"] != params_j["noise"] or
                    params_i["scale"] != params_j["scale"] or
                    params_i["contrast"] != params_j["contrast"]
                )
    ...
    def test_distributed_performance_and_scalability(self):
        ...
        for num_samples, world_size in test_configs:
            ...
            assert generation_time < 10.0, f"Generation took too long: {generation_time:.2f}s for {num_samples} samples, {world_size} ranks"
