...
class TestBenchmarkingIntegration:
    ...
    def test_cross_configuration_comparison_accuracy(self):
        ...
        for config_name, metrics in report.metrics_comparison.items():
            ...
            if baseline_throughput is None:
                ...
                baseline_entry = next((entry for entry in history if entry['config_name'] == 'small_dataset'), None)
                if baseline_entry:
            ...
            if baseline_throughput and baseline_throughput > 0:
                expected_change = ((current_throughput - baseline_throughput) / baseline_throughput) * 100
    ...
    def test_benchmark_integration_with_distributed_functions(self):
        ...
        def extended_benchmark_generation(num_samples, config=None, **kwargs):
            if kwargs.get('use_distributed'):
                ...
                throughput = (rank_samples * runner.config.iterations) / total_time if total_time > 0 else 0
    ...
    def test_benchmark_error_handling_integration(self):
        ...
        invalid_configs = [
            {'name': 'invalid', 'type': 'unknown_type', 'num_samples': 100}
        ]
