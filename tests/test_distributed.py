...
from src.dpa import AugmentationConfig, gen_augmentation_params
...
class TestDistributedRangeSplitter:
    ...
    def test_get_rank_for_sample_invalid_sample_id_too_large(self):
        ...
        with pytest.raises(ValueError, match="sample_id \\(100\\) must be < total_samples \\(100\\)"):
        ...
        for total_samples, world_size in test_cases:
            ...
            for start_id, end_id in ranges:
                for sample_id in range(start_id, end_id):
                    # Check for overlaps
                    assert sample_id not in covered_samples, f"Sample {sample_id} covered by multiple ranges"
                    covered_samples.add(sample_id)
            
            # Check that all samples are covered exactly once
            expected_samples = set(range(total_samples))
            assert covered_samples == expected_samples, f"Coverage mismatch for {total_samples} samples, {world_size} ranks"
    ...
    def test_range_size_distribution(self):
        ...
        for total_samples, world_size, expected_sizes in test_cases:
            ...
            assert actual_sizes == expected_sizes, f"Size distribution mismatch for {total_samples} samples, {world_size} ranks"
...
class TestDistributedStreamingFunctions:
    ...
    def test_distributed_streaming_integration_with_range_splitter(self):
        ...
        for rank in range(world_size):
            ...
            assert actual_count == expected_count, f"Rank {rank}: expected {expected_count}, got {actual_count}"
