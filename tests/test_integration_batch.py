...
class TestBatchProcessingIntegration:
    ...
    def test_integration_with_existing_streaming_api(self):
        ...
        for strategy in strategies:
            ...
            assert total_items == num_samples, f"Item count mismatch for strategy {strategy}: expected {num_samples}, got {total_items}"
    ...
    def test_memory_limit_enforcement_during_processing(self, mock_memory):
        ...
        memory_progression = [
            ...
            MemoryInfo(available_mb=100, used_mb=1900, total_mb=2048, percent_used=95.0),  # Critical
        ]
    ...
    def test_memory_aware_batch_size_adjustment(self, mock_memory):
        ...
        memory_states = [
            MemoryInfo(available_mb=200, used_mb=1800, total_mb=2048, percent_used=88.0),  # High usage
            MemoryInfo(available_mb=500, used_mb=1500, total_mb=2048, percent_used=73.0),  # Medium usage
            MemoryInfo(available_mb=1200, used_mb=800, total_mb=2048, percent_used=39.0),  # Low usage
        ]
    ...
    def test_performance_under_different_batching_strategies(self):
        ...
        for strategy, config_params in strategy_configs:
            ...
            # Verify correctness
            total_items = sum(len(batch) for batch in batches)
            assert total_items == num_samples, f"Strategy {strategy} processed {total_items} items, expected {num_samples}"
            ...
            assert processing_time < 5.0, f"Strategy {strategy} took too long: {processing_time:.2f}s"
            
            # Throughput should be reasonable (at least 100 items/sec)
            assert throughput > 100, f"Strategy {strategy} throughput too low: {throughput:.2f} items/sec"
        ...
        for memory_limit in memory_limits:
            ...
            for batch in batches:
                if len(batch) < batch_config.min_batch_size:
                    ...
                    assert batch == batches[-1], f"Non-final batch too small: {len(batch)} < {batch_config.min_batch_size}"
    ...
    def test_batch_processing_with_large_datasets(self):
        ...
        for size in dataset_sizes:
            ...
            assert throughput > 500, f"Throughput too low for size {size}: {throughput:.2f} items/sec"
            
            # Processing time should be reasonable (less than 30 seconds for largest dataset)
            assert processing_time < 30.0, f"Processing time too long for size {size}: {processing_time:.2f}s"
    ...
    def test_memory_monitor_limit_exceeded(self, mock_get_memory):
        ...
        mock_get_memory.side_effect = [
            ...
            MemoryInfo(available_mb=400, used_mb=600, total_mb=2048, percent_used=30.0)    # Final (exceeds 500MB limit)
        ]
    ...
    def test_integration_with_real_dpa_streaming_large_scale(self):
        ...
        for strategy in strategies_to_test:
            ...
            throughput = num_samples / processing_time if processing_time > 0 else float('inf')
            assert throughput > 200, f"Strategy {strategy} throughput too low: {throughput:.2f} items/sec"
            assert processing_time < 15.0, f"Strategy {strategy} took too long: {processing_time:.2f}s"