...
def measure_memory() -> Generator[dict[str, int], None, None]:
    ...
    try:
        ...
        result['delta_system_mb'] = result['final_system_used_mb'] - result['initial_system_used_mb']
        ...
        for i in range(iterations):
            ...
            with measure_time() as timer, measure_memory() as memory:
                try:
                    ...
                    raise MeasurementError(f"Function failed during benchmarking iteration {i}: {e}") from e
...
class PerformanceProfiler:
    ...
    def start_profiling(self, operation_name: str) -> None:
        ...
        try:
            ...
            raise ProfilingError(f"Failed to start profiling operation '{operation_name}': {e}") from e
    ...
    def end_profiling(self, operation_name: str) -> ProfileResult:
        ...
        try:
            ...
            raise ProfilingError(f"Failed to end profiling operation '{operation_name}': {e}") from e
    ...
    def profile_function(self, func: Callable[P, T]) -> Callable[P, T]:
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
            operation_name = f"{func.__module__}.{func.__name__}" if hasattr(func, '__module__') else func.__name__
...
class BenchmarkRunner:
    ...
    def benchmark_generation(self, num_samples: int, config: Any = None) -> PerformanceMetrics:
        ...
        try:
            ...
            from .dpa import gen_augmentation_params, AugmentationConfig
            ...
            total_time = sum(times)
            avg_time = statistics.mean(times)
            throughput = (num_samples * self.config.iterations) / total_time if total_time > 0 else 0.0
    ...
    def benchmark_streaming(self, num_samples: int, chunk_size: int = 1000) -> PerformanceMetrics:
        ...
        try:
            ...
            from .dpa import stream_augmentation_chain, AugmentationConfig
            ...
            for _ in range(self.config.warmup_iterations):
                try:
                    ...
                    warmup_stream = stream_augmentation_chain(min(100, num_samples), config, chunk_size=chunk_size)
            ...
            for i in range(self.config.iterations):
                ...
                with measure_time() as timer:
                    if self.config.measure_memory:
                        with measure_memory() as memory:
                            stream = stream_augmentation_chain(num_samples, config, chunk_size=chunk_size)
    ...
    def benchmark_batch_processing(self, num_samples: int, batch_config: Any = None) -> PerformanceMetrics:
        ...
        try:
            ...
            from .batch import BatchProcessor, BatchStrategy, BatchConfig
            from .dpa import stream_augmentation_chain, AugmentationConfig
    ...
    def compare_configurations(self, configs: list[dict]) -> ComparisonReport:
        ...
        try:
            ...
            for config_name, metrics in results.items():
                ...
                throughput_change = ((metrics.throughput_samples_per_second - baseline_metrics.throughput_samples_per_second) 
                                   / baseline_metrics.throughput_samples_per_second * 100) if baseline_metrics.throughput_samples_per_second > 0 else 0.0
                
                latency_change = ((metrics.avg_latency_ms - baseline_metrics.avg_latency_ms) 
                                / baseline_metrics.avg_latency_ms * 100) if baseline_metrics.avg_latency_ms > 0 else 0.0
                
                memory_change = ((metrics.memory_usage_mb - baseline_metrics.memory_usage_mb) 
                               / baseline_metrics.memory_usage_mb * 100) if baseline_metrics.memory_usage_mb > 0 else 0.0
                
                cpu_change = ((metrics.cpu_usage_percent - baseline_metrics.cpu_usage_percent) 
                            / baseline_metrics.cpu_usage_percent * 100) if baseline_metrics.cpu_usage_percent > 0 else 0.0
                ...
                improvement_score = (throughput_change - latency_change - memory_change - cpu_change) / 4
                performance_improvements[config_name] = improvement_score
            
            # Generate recommendations
            recommendations = self._generate_recommendations(results, metrics_comparison, performance_improvements)
    ...
    def _generate_recommendations(self, results: dict[str, PerformanceMetrics], 
        ...
        if performance_improvements:
            best_config = max(performance_improvements.keys(), key=lambda k: performance_improvements[k])
            best_score = performance_improvements[best_config]
            
            if best_score > 10:  # Significant improvement
                recommendations.append(f"Configuration '{best_config}' shows significant performance improvement ({best_score:.1f}% overall)")
            elif best_score > 0:
                recommendations.append(f"Configuration '{best_config}' shows modest performance improvement ({best_score:.1f}% overall)")
        ...
        for config_name, comparison in metrics_comparison.items():
            ...
            if throughput_change > 20:
                recommendations.append(f"Configuration '{config_name}' significantly improves throughput by {throughput_change:.1f}%")
            elif throughput_change < -20:
                recommendations.append(f"Configuration '{config_name}' significantly reduces throughput by {abs(throughput_change):.1f}% - consider alternatives")
            
            # Latency recommendations
            if latency_change < -20:
                recommendations.append(f"Configuration '{config_name}' significantly reduces latency by {abs(latency_change):.1f}%")
            elif latency_change > 20:
                recommendations.append(f"Configuration '{config_name}' increases latency by {latency_change:.1f}% - may impact responsiveness")
            
            # Memory recommendations
            if memory_change < -20:
                recommendations.append(f"Configuration '{config_name}' significantly reduces memory usage by {abs(memory_change):.1f}%")
            elif memory_change > 50:
                recommendations.append(f"Configuration '{config_name}' increases memory usage by {memory_change:.1f}% - monitor for memory constraints")
            
            # CPU recommendations
            if cpu_change < -20:
                recommendations.append(f"Configuration '{config_name}' significantly reduces CPU usage by {abs(cpu_change):.1f}%")
            elif cpu_change > 50:
                recommendations.append(f"Configuration '{config_name}' increases CPU usage by {cpu_change:.1f}% - may impact system performance")
        
        # General recommendations based on patterns
        baseline_metrics = list(results.values())[0]  # First result is baseline
        
        if baseline_metrics.memory_usage_mb > 1000:
            recommendations.append("High memory usage detected - consider using batch processing with memory limits")
        
        if baseline_metrics.avg_latency_ms > 100:
            recommendations.append("High latency detected - consider optimizing augmentation parameters or using streaming")
        
        if baseline_metrics.throughput_samples_per_second < 100:
            recommendations.append("Low throughput detected - consider batch processing or parameter optimization")
        
        # Remove duplicates and limit recommendations
        recommendations = list(dict.fromkeys(recommendations))  # Remove duplicates while preserving order
        return recommendations[:10]  # Limit to top 10 recommendations
    
    def generate_performance_report(self, comparison_report: ComparisonReport, 
...
def format_benchmark_result(result: BenchmarkResult, format_type: str = "text") -> str:
    ...
    if format_type == "json":
        ...
        return (f"{result.function_name},{result.iterations},{result.avg_time_ms:.3f},"
