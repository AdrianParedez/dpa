...
class DistributedRangeSplitter:
    ...
    def get_rank_range(self, rank: int) -> tuple[int, int]:
        ...
        try:
            ...
            if rank < remainder:
                ...
                start_id = (
                    remainder * (base_samples_per_rank + 1)
                    + (rank - remainder) * base_samples_per_rank
                )
    ...
    def get_rank_for_sample(self, sample_id: int) -> int:
        ...
        if sample_id >= self.total_samples:
            raise ValueError(
                f"sample_id ({sample_id}) must be < total_samples ({self.total_samples})"
            )
...
def stream_distributed_augmentation_chain(
    ...
    try:
        ...
        if verbose:
            print(
                f"Rank {rank}/{world_size}: processing samples {start_id}-{end_id - 1} ({rank_samples} samples)"
            )
            ...
            for sample_id in range(start_id, actual_end):
                try:
                    ...
                    raise SeedGenerationError(
                        f"Failed to generate parameters for rank {rank}, sample {sample_id}: {e}"
                    ) from e
        else:
        ...
        if isinstance(e, (ValueError, RangeSplittingError, SeedGenerationError)):
            raise
        raise RangeSplittingError(
            f"Failed to stream distributed augmentation chain for rank {rank}: {e}"
        ) from e
...
def stream_distributed_augmentation_range(
    ...
    if verbose:
        print(
            f"Rank {rank}/{world_size}: processing range {start_id}-{end_id - 1} ({num_samples} samples)"
        )
...
def gen_distributed_augmentation_params(
    ...
    try:
        ...
        raise SeedGenerationError(
            f"Failed to generate distributed augmentation parameters: {e}"
        ) from e
